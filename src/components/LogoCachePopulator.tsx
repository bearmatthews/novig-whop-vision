...
export function LogoCachePopulator() {
  ...
  const populateLogos = async (league?: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('populate-college-logos', {
        body: { league, all: !league }
      });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          College Sports Logo Cache
        </CardTitle>
        <CardDescription>
          Pre-populate the logo cache with major college teams
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          <Button
            onClick={() => populateLogos()}
        ...
        {results && (
          <div className="mt-4 space-y-2 text-sm">
            {Object.entries(results).map(([league, data]: [string, any]) => (
              <div key={league} className="p-3 rounded-lg bg-muted">
